...
* Preamble
This is my Emacs configuration, written in Org Mode. Upon saving this file, all of the following code blocks are added to ~init.el~ using ~org-babel-tangle~. I got the code snippet for this, which you can find [[https://config.daviwil.com/emacs][here]], from David Wilson's 'Emacs from Scratch' GitHub repository. I also got the snippet in the 'Startup' section from this same respository. Needless to say, the System Crafters channel has been a helpful resource for compiling my own Emacs configuration.
