...
*** project.el
For some reason, ~project-prefix-map~ wasn't getting bound to ~C-x p~. In fact, ~project-prefix-map~ wasn't even defined in project.el. Adding a use-package declaration seems to have fixed the problem. Perhaps I had an old version installed?
#+begin_src emacs-lisp
