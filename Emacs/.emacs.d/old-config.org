...
* Basic Configuration
** Startup
Improve startup speed by reducing the number of garbage collections during startup. The value of ~gc-cons-threshold~ is restored after startup for safety. See [[https://nullprogram.com/blog/2016/12/22/][this]] article for the advantages of lexical scope in Emacs Lisp.
...
* User Interface
** Improved Scrolling
See [[https://www.emacswiki.org/emacs/SmoothScrolling]] for details on this topic.
#+begin_src emacs-lisp
...
*** Suggestion Pop-Up For Prefix Keys
#+begin_src emacs-lisp
...
** Embark
Embark provides a keyboard-operated context menu, which is activated by calling the ~embark-act~ function. This will open the ~*Embark Actions*~ buffer, which lists the actions that can be performed on the current target. An action will be executed when its corresponding key sequence is pressed. A target could be a URL, sentence, file path, symbol, buffer, etc. See [[https://github.com/oantolin/embark]] for more details.
#+begin_src emacs-lisp
  (use-package embark
    :bind
    ("C-."   . embark-act)
...
*** Minibuffer
**** Marginalia
~marginalia-mode~ adds annotations to completion candidates in the minibuffer.
#+begin_src emacs-lisp
...
When ~savehist-mode~ is enabled, minibuffer history will persist over Emacs restarts.
#+begin_src emacs-lisp
...
**** Vertico
Vertico is an extensible minibuffer completion UI. Completion candidates will be sorted by their history positions when ~savehist-mode~ is enabled.
#+begin_src emacs-lisp
  (use-package vertico :init (vertico-mode))
#+end_src

*** Completion Styles
~orderless~ is a completion style that can match multiple space-separated components in any order.
#+begin_src emacs-lisp
...
*** Managing Projects
When I was running Emacs 27.1, ~project-prefix-map~ was not defined in ~project.el~, so I couldn’t bind it to a key sequence. I fixed this by adding the following ~use-package~ form, although I no longer have this issue with Emacs 29.0.50.
#+begin_src emacs-lisp
  ;; (use-package project)
...
* Editing Behaviour
** Backspace Tab Characters Entirely
#+begin_src emacs-lisp
...
** Tree Sitter
I use ~tree-sitter-hl-mode~ for syntax-highlighting, which is faster than the default regexp-based ~font-lock-mode~; this is especially noticable while editing large files. If the value of ~tree-sitter-hl-default-patterns~ in a given buffer is ~nil~, ~tree-sitter-hl-mode~ will /not/ provide syntax highlighting.
#+begin_src emacs-lisp
...
*** Pomodoro Timer
Just call ~org-timer-set-timer~! I don't think there's a point in timing the break periods, since I'll probably be away from my workstation at these times.
#+begin_src emacs-lisp
